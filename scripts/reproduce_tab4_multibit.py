"""Reproduce Table 4 (tab:multibit): multi-bit quantization (IVFPQ / OPQ) results.
...
Outputs:
  ...
  - results/tab4_multibit_reproduced__np16_k0.csv
"""
...
def output_csv_path(nprobe: int, rerank_k: int) -> Path:
    return RESULTS_DIR / f"tab4_multibit_reproduced__np{nprobe}_k{rerank_k}.csv"
...
def print_latex_rows(records: list[dict[str, str]]) -> None:
    """Emit the 4 direction-averaged rows matching current paper Table 4 (tab:multibit).
    ...
    print("\n% --- auto-generated by reproduce_tab4_multibit.py (direction-averaged) ---")
    print("% Table 4 (tab:multibit): 4 rows, averaged over t->i and i->t")
