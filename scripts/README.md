...
| `builders/` | FAISS-bound emitters that produced the committed measurements (`emit_*`), the LAION ground-truth builder, and feature-bound rebuilds (`reproduce_gap_energy.py`, LAION / AudioCaps / Clotho re-evals). Run offline; never required for the cached reproduce loop. `emit_signbit_metrics.py` is research-tree-only in both its paths (R15 measurement and `--skip-run` reformat); the committed `results/sources/mechcontrol_metrics.csv` is the canonical record here. | `data/features/`, `laion_dir` (`config/paths.yaml`; `PMC_LAION_DIR` / `LAION400M_DIR` env) |
