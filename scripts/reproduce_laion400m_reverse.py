"""
reproduce_laion400m_reverse.py -- PMC-RaBitQ on LAION-400M, REVERSE direction (image→text).

This is the reverse of 11_reproduce_laion400m.py:
  Forward:  text queries  against  image DB  (text→image)
  Reverse:  image queries against  text DB   (image→text)

Runs 3 methods (vanilla_rabitq, pmc_1.00, vanilla_rabitq_meanshift) across
nprobe sweep [1, 2, 4, 8, 16, 32, 64, 128, 256] with proper timing (single-thread,
5 runs, median). Handles the 400M scale memory constraint by:
  - Reading TEXT embeddings as 410 .npy shards (~1M vectors each, float16) as DB
  - Computing ground truth shard-by-shard and merging top-100 per shard
  - Training on a 5M sample from the first 5 text shards
  - Adding vectors shard-by-shard and freeing each after add
  - Queries: 10K random samples from img_emb_0.npy (first image shard, ~1M)

Data layout:
  E:/laion400m/img_emb/img_emb_{0..409}.npy   float16 (N_shard, 512)
  E:/laion400m/text_emb/text_emb_{0..9}.npy   float16 (N_shard, 512)

Gap direction: mean(img_queries) - mean(text_db_sample)
  = negative of the forward gap (mean(text_queries) - mean(img_db_sample))
  This is correct — compute_gap(db_sample, queries) with db=text, queries=img
  naturally yields mean(img) - mean(text).

Output: results/pmc_laion400m_reverse_nlist80k_seed42.csv

Usage
-----
  python scripts/scale/13_reproduce_laion400m_reverse.py             # full reverse experiment
  python scripts/scale/13_reproduce_laion400m_reverse.py --summary   # print CSV summary and exit
  python scripts/scale/13_reproduce_laion400m_reverse.py --gt-only   # compute and save GT only
"""
...
# ---------------------------------------------------------------------------
SEED = 42
N_TEXT_SHARDS = 410   # text embeddings are the DB (410 shards)
N_IMG_SHARDS = 10     # image embeddings are the query source (10 shards)
N_QUERIES = 10_000
TOP_K = 100
NLIST = 80_000          # ~4*sqrt(407M); paper operating point
...
# ---------------------------------------------------------------------------
# Query loading (from first image shard) — REVERSE: img queries
# ---------------------------------------------------------------------------

def load_queries() -> np.ndarray:
    """Load 10K query vectors sampled from img_emb_0.npy.

    Loads ~1M vectors from the first image shard (2 GB in float32), samples
    ...
    """
    print(f"\n[queries] Loading img_emb_0.npy ...")
    t0 = time.perf_counter()
    img_shard = load_shard("img", 0)   # float32 after cast
    img_norm = l2_normalize(img_shard)
    del img_shard

    np.random.seed(SEED)
    query_idx = np.random.choice(len(img_norm), size=N_QUERIES, replace=False)
    queries = img_norm[query_idx].copy()
    del img_norm
    gc.collect()
...
# ---------------------------------------------------------------------------
# Ground truth — shard-by-shard IndexFlatIP over TEXT shards (REVERSE)
# ---------------------------------------------------------------------------
...
def compute_gt_shards(
    ...
    """Compute brute-force ground truth over all text shards (image→text direction).
    ...
    for shard_idx in range(n_text_shards):
        if not shard_exists("text", shard_idx):
            print(f"  [GT] shard {shard_idx} missing -- stopping at {shard_idx} shards")
            break

        t0 = time.perf_counter()
        print(
            f"  [GT] shard {shard_idx + 1}/{n_text_shards}  "
            f"offset={cumulative_offset:,} ...",
            end=" ",
            flush=True,
        )
        shard_raw = load_shard("text", shard_idx)
...
def load_or_compute_gt(
    ...
    print(
        ...
        f"(Q={len(queries)}, top_k={top_k}, shards={n_text_shards}) ..."
    )
    t0 = time.perf_counter()
    gt, n_db = compute_gt_shards(queries, n_text_shards, top_k)
    elapsed = time.perf_counter() - t0
...
# ---------------------------------------------------------------------------
# Gap estimation — REVERSE: load text shards as DB sample
# ---------------------------------------------------------------------------

def estimate_gap(queries: np.ndarray, n_shards: int = GAP_SAMPLE_SHARDS) -> np.ndarray:
    """Estimate modality gap from first n_shards TEXT shards (reverse direction).

    Loads ~5M text vectors (float32), computes mean, then discards.
    Gap = mean(img_queries) - mean(text_db_sample)

    Returns
    -------
    gap : (d,) float32  =  mean(img_queries) - mean(text_db_sample)
    """
    print(f"\n[Gap] Loading {n_shards} text shards for gap estimation ...")
    t0 = time.perf_counter()
    parts = []
    for i in range(n_shards):
        shard_raw = load_shard("text", i)
...
# ---------------------------------------------------------------------------
# Shard-based index building — REVERSE: text shards are DB
# ---------------------------------------------------------------------------
...
def build_index_shards(
    ...
    """Build an IVFRaBitQFastScan index by training on a text shard sample then
    adding all text shards one at a time (image→text reverse direction).

    Parameters
    ----------
    n_text_shards : total number of text shards to add
    ...
    # --- Train phase ---------------------------------------------------------
    print(f"\n[{label}] Loading {train_shards} text shards for training ...")
    t0 = time.perf_counter()
    parts = []
    ...
    for shard_idx in range(n_text_shards):
        ...
        t0 = time.perf_counter()
        print(
            f"[{label}] Adding shard {shard_idx + 1}/{n_text_shards} ...",
            end=" ",
            flush=True,
        )
        shard_raw = load_shard("text", shard_idx)
...
def check_laion400m_reverse_csv_summary() -> bool:
    """Print a compact summary of LAION-400M reverse R@100 from result CSV.
    ...
    """
    csv_path = RESULTS_DIR / f"pmc_laion400m_reverse_nlist80k_seed{SEED}.csv"
    if not csv_path.exists():
        print(f"  SKIP: {csv_path} not found")
        return False

    print("\n=== LAION-400M REVERSE (image→text) CSV summary ===")
...
def main(gt_only: bool = False) -> None:
    ...
    print("=" * 70)
    print("PMC-RaBitQ  --  LAION-400M  REVERSE (image→text)  N~400M  d=512  (single-thread search, multi-thread build)")
    ...
    # Count available text shards (DB in reverse direction)
    n_text_available = sum(
        1 for i in range(N_TEXT_SHARDS) if shard_exists("text", i)
    )
    print(f"\n[data] Text shards available (DB): {n_text_available}/{N_TEXT_SHARDS}")
    if n_text_available == 0:
        print("[ERROR] No text shards found. Run download_laion400m.py first.")
        sys.exit(1)

    # Paths for ground truth and results
    gt_path = LAION_DIR / f"groundtruth.laion400m.reverse.{N_QUERIES // 1000}K.npy"
    out_csv = RESULTS_DIR / f"pmc_laion400m_reverse_nlist80k_seed{SEED}.csv"

    # Load queries (image embeddings, reverse direction)
    queries = load_queries()
    d = queries.shape[1]

    # Ground truth (image queries vs text DB)
    gt, n_db = load_or_compute_gt(
        gt_path=gt_path,
        queries=queries,
        n_text_shards=n_text_available,
        top_k=TOP_K,
    )
    ...
    # Gap estimation (uses first 5 text shards as DB sample)
    # gap = mean(img_queries) - mean(text_db_sample)
    ...
    # ------------------------------------------------------------------
    # Build vanilla index (text DB, no shift)
    # ------------------------------------------------------------------
    print(f"\n{'=' * 70}")
    vanilla_cache = INDEX_CACHE_DIR / f"reverse_vanilla_rabitq_nlist{NLIST}_seed{SEED}.index"
    if vanilla_cache.exists():
        ...
        print("[vanilla_rabitq] Building index (text DB, reverse direction) ...")
    ...
    # ------------------------------------------------------------------
    print(f"\n{'=' * 70}")
    print("[vanilla_rabitq] nprobe sweep (image→text) ...")
    ...
    # ------------------------------------------------------------------
    print(f"\n{'=' * 70}")
    print("[vanilla_rabitq_meanshift] nprobe sweep (query-side shift, vanilla index, image→text) ...")
    for nprobe in NPROBE_VALUES:
        _, retrieved, qps = timed_search(
    ...
    # ------------------------------------------------------------------
    # Build PMC index (alpha=1.00, text DB shifted toward image query center)
    # ------------------------------------------------------------------
    alpha = 1.0
    method_name = f"pmc_{alpha:.2f}"
    print(f"\n{'=' * 70}")
    pmc_cache = INDEX_CACHE_DIR / f"reverse_{method_name}_nlist{NLIST}_seed{SEED}.index"
    if pmc_cache.exists():
        ...
        print(f"[{method_name}] Building PMC-shifted index (alpha={alpha}, text DB, reverse direction) ...")
        _set_threads(os.cpu_count() or 8)
    ...
    # ------------------------------------------------------------------
    print(f"\n{'=' * 70}")
    print(f"[{method_name}] nprobe sweep (queries unchanged at alpha=1, image→text) ...")
    for nprobe in NPROBE_VALUES:
        _, retrieved, qps = timed_search(
    ...
    # ------------------------------------------------------------------
    print(f"\n{'=' * 100}")
    print(f"SUMMARY TABLE  --  LAION-400M REVERSE (image→text)  N={n_db:,}  d={d}  PMC-RaBitQ")
    print("=" * 100)
    ...
    print(f"\n[laion400m_reverse] CSV written -> {out_csv}")
    print(f"[laion400m_reverse] Done. {len(all_records)} records.")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="PMC-RaBitQ LAION-400M reverse experiment (image→text)"
    )
    parser.add_argument(
        "--summary",
        action="store_true",
        help="Print LAION-400M reverse CSV summary and exit.",
    )
    parser.add_argument(
        "--gt-only",
        action="store_true",
    ...
    if args.summary:
        ok = check_laion400m_reverse_csv_summary()
