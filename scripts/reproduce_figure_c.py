...
DATASETS:
  ...
  Clotho eval -- ImageBind (d=1024)        directions: t2a, a2t
...
def main() -> None:
    ...
    # ------------------------------------------------------------------
    # Clotho: 1045 audio clips / 1045 text captions (eval split)
    # t→a: DB=audio(1045), Q=text(1045)
    # a→t: DB=text(1045), Q=audio(1045)
    print("\n=== Clotho eval (ImageBind, d=1024) ===")
    cl_audio = np.load(DATA_DIR / "clotho_eval_imagebind_audio_seed42.npy")
    cl_text = np.load(DATA_DIR / "clotho_eval_imagebind_text_seed42.npy")
    print(f"  audio: {cl_audio.shape}, text: {cl_text.shape}")
