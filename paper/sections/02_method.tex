...
Let $\phi$ be a frozen multimodal encoder mapping modalities $a$
and $b$ into $\mathbb{R}^d$.
The database $\mathcal{X} = \{\phi(x_i)\}$ holds modality-$b$ embeddings;
queries $q = \phi(q_j)$ come from modality~$a$.
With per-modality means $\boldsymbol{\mu}_q = \mathbb{E}[\phi(q)]$
and $\boldsymbol{\mu}_x = \mathbb{E}[\phi(x)]$,
the \emph{modality gap} is $\mathbf{g} = \boldsymbol{\mu}_q - \boldsymbol{\mu}_x$.
The goal is a compressed ANN index over $\mathcal{X}$ that answers
...
\end{equation}
a density-weighted $\ell_1$ norm of the gap. Eq.~\eqref{eq:ratio-form} shows that flip risk in dimension~$i$ grows with $|\alpha g_i|/|\delta_i|$. When gap energy is concentrated---in CLIP-L on MSCOCO, top 10\% of dimensions carry ${\approx}90$\% of $\|\mathbf{g}\|^2$---corruption is \emph{structured} rather than random, biasing Hamming distance estimates systematically. The same offset misaligns IVF cluster assignments, compounding routing errors with code corruption. Eq.~\eqref{eq:expected-flips} formalizes this scaling, explaining the gap-proportional gains in Table~\ref{tab:mainresults} and the top-5\% selective PMC result on concentrated-gap encoders (Figure~\ref{fig:analysis-bcd}b).
...
construction.
From a training sample of modality-$a$ queries and the modality-$b$
database, we compute the gap $\mathbf{g}$ and shift database vectors
...
$\alpha{=}1$, query-time transformation is
identity ($\mathbf{q}'{=}\mathbf{q}$), so serving incurs no extra cost.

\textbf{Effectiveness of {\boldmath$\alpha{=}1$}.}
Per-vector renormalization after the shift preserves unit-norm structure; the $\alpha$-sweep in Figure~\ref{fig:analysis-bcd}a confirms $\alpha{=}1$ as best or near-best in all settings.\looseness=-1
...
\begin{equation}\label{eq:selective}
  ...
  \end{cases}
  \,,\qquad
  \mathbf{x}'_{\mathrm{sel}} = \frac{\mathbf{x} + \alpha \, \mathbf{g}_{\mathrm{sel}}}{\|\mathbf{x} + \alpha \, \mathbf{g}_{\mathrm{sel}}\|}
\end{equation}
...
\end{equation}
Since flip risk concentrates in high-$|g_i|$ dimensions (Eq.~\eqref{eq:ratio-form}), correcting only the highest-energy ones suffices: for CLIP the top 10\% by $|g_i|$ carry 86--92\% of total gap energy, so $P{=}5$\% already recovers peak recall; ImageBind's diffuse gap (${\approx}72\%$) requires full-vector correction.\looseness=-1
