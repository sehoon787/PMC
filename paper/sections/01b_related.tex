...
structures, or online serving updates. PMC is complementary and not a direct
baseline: one offline centroid correction in the 1-bit compressed regime, with
no added index-time memory or required retraining.
...
{\scriptsize $q{\to}db$: text-to-image/audio; $db{\to}q$: reverse. CL-L\,=\,CLIP-L; IB\,=\,ImageBind; $\|\mathbf{g}\|$\,=\,modality-gap norm; MeanShift\,=\,query-only mean shift. $n_\mathrm{probe}$ split $q{\to}db$/$db{\to}q$ where directions differ (AudioCaps, Clotho).}
\end{table*}
