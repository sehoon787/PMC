...
bottleneck: high-dimensional float32 embeddings can require terabytes of RAM,
...
dimension as one sign bit relative to the dataset centroid. Its adoption in Apache Lucene BBQ and multi-bit
Extended-RaBitQ~\cite{gao2025extended} shows BQ indexing is now
deployed in production.\looseness=-1
...
vectors share a common centroid. Cross-modal
...
scales with gap magnitude (Figure~\ref{fig:modality-gap}); an oracle test (MSCOCO, CLIP-L) confirms this: same-modality queries ($\|\mathbf{g}\|{=}0$) reach R@100$=$0.71 under RaBitQ, versus 0.54 for cross-modal queries ($\|\mathbf{g}\|{=}0.82$).
...
In this paper, we analyze how the modality gap degrades BQ ANN indexes at two coupled stages---IVF routing and binary
code formation---and, guided by this analysis, propose Per-Modality Centroid Correction
